...
const Book = ({ book, onFav }) => {
  const { id, image, title, author, publish_date, rating, isFavorite } = book;
  return (
    <div className="space-y-3 h-[450px] relative">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 text-xs lg:text-sm absolute bottom-0">
          ...
          {isFavorite ? (
            <button
              class="flex min-w-[132px] items-center justify-center gap-1 rounded-md bg-[#DC2954]/[14%] py-1.5 text-[#DC2954] transition-all hover:bg-[#DC2954]/[24%] lg:py-1.5"
              onClick={() => onFav(id)}
            >
              <svg
                width="16"
